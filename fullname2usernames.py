...
def sanitize_name(name):
    """
    Sanitize a name by removing accents only.
    
    Args:
        name (str): The name to sanitize
        
    Returns:
        str: The name with accents removed but other characters preserved
    """
    if not name:
        return ""
    ...
    # Keep only non-accent characters (removes accents but keeps everything else)
    name = ''.join(char for char in name if unicodedata.category(char) != 'Mn')
    
    return name

def generate_usernames(fullname, formats_to_generate):
    sanitized_fullname = sanitize_name(fullname)
    
    try:
        firstname, lastname = sanitized_fullname.strip().split()
    except ValueError:
        return {}
